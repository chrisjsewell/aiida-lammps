...
def generate_LAMMPS_input(parameters,
    ...
    initial_temp, _, _ = get_path(
        pdict, ["integration", "constraints", "temp"],
    ...
    lammps_input_file += 'fix             int all {0} {1} {2}\n'.format(
        ...
        join_keywords(
            get_path(
                pdict, ["integration", "constraints"], {}, raise_error=False)),
        join_keywords(
            get_path(
                pdict, ["integration", "keywords"], {}, raise_error=False)))
    ...
    lammps_input_file += 'dump            aiida all custom {0} {1} element x y z\n'.format(
        parameters.dict.dump_rate, trajectory_file)
...
class MdCalculation(BaseLammpsCalculation, JobCalculation):
    ...
    def _init_internal_params(self):
        ...
        self._retrieve_temporary_list = [
            self._OUTPUT_TRAJECTORY_FILE_NAME, self._INPUT_UNITS
        ]
